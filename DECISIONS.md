...
## D-020 — Rebranding a Propi: solo capa visible, identificadores de máquina intactos

**Contexto.** El cliente pidió un rebranding completo: EricPay pasa a llamarse Propi, con nuevo logo (P naranja con pájaro), nueva paleta (naranja/marrón sobre neutros cálidos, tokens en DESIGN.md) y conservando los gradientes como elemento de identidad.

**Decisión.** El rebranding cubre todo lo visible (nombre de app, wordmark, copy, colores, logo, iconos, splash) pero **no toca ningún identificador de máquina**: el scheme de deep links y OAuth (`ericpay`), el campo `app: 'ericpay'` del protocolo QR, el slug de EAS, el bundleId/package (`com.andresstbn.ericpay`) y el proyecto de Firebase (`ericpay-7626c`) se conservan.

**Por qué.** Cambiar el scheme rompería el login social (redirect URI registrado en Google/Meta/Firebase) y los deep links de invitación a grupos; cambiar el campo `app` del QR invalidaría todos los QRs personales/reutilizables ya emitidos e impresos; cambiar slug/bundleId desvincula el proyecto EAS y obliga a re-aprovisionar credenciales. Nada de eso es visible para el usuario y el encargo exigía no dañar funcionalidad. Si en el futuro se quiere migrar el scheme, será un incremento propio con periodo de doble aceptación en el parser.

**Además.** El arte original del logo venía sin canal alpha; el recorte automático de fondo (macOS) además vació el interior blanco del pájaro, cuyo borde exterior es blanco-sobre-blanco y no se puede recuperar por flood-fill. Se restauró con un relleno geométrico (pixel transparente con naranja arriba en su columna y a la derecha en su fila = interior del pájaro), que respeta el hueco cóncavo real bajo el bowl de la P. De ese raster (`assets/brand/propi-mark.png`, 1024 con alpha) se generan icono, splash, adaptive icons Android y favicon; no hay fuente vectorial, el master es el PNG del cliente. Los tokens de color se renombraron mecánicamente (`navy900/blue600 → brown700`, `navy700 → orange500`, `cyan400 → orange400`, `yellow* → peach*`); `blue600` y `navy900` colapsan en un solo token porque en el nuevo sistema el primario y el color de marca oscuro son el mismo (#964900). El botón de Facebook dejó de usar un token del tema: su azul es marca de Facebook, no de Propi. Se eliminó también el badge "DEMO" y el copy de "dinero ficticio" del login, que violaban la regla de copy de CLAUDE.md.
